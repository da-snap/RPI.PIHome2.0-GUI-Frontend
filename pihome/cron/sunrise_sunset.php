...
// Set timezone
error_reporting(0);
require_once '/var/www/pihome/config/dbconfig.inc.php';
require_once '/var/www/pihome/cron/functions.inc.php';
...
if($now >= $sunset AND $now <= $ssfd)
{
    ...
    foreach($lamps as $lampID)
    {
        ...
        if($stat=="0")
        {
            ...
            shell_exec('sudo ' . SEND_PATH . '/send '.$code.' '.$letter.' '.$status.' ');
